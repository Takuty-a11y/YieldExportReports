...
using YieldExportReports.Database.DBOperates.MSSQLServer;

namespace YieldExportReports.Database.DBOperates.MSOLEDB
{
    public class SQLOperator : ISQLOperator
    {
        ...
        public DBObjectCollection GetDBObjectsToList(IDbConnection dbConnection)
        {
            ...
            try
            {
                var c = (OleDbConnection)dbConnection;
                c.InfoMessage += new OleDbInfoMessageEventHandler(c_InfoMessage);

                var dtTbl = c.GetSchema(OleDbMetaDataCollectionNames.Tables);
        ...
        /// <param name="c"></param>
        /// <returns></returns>
        private List<DBObject> GetTableKeys(string tblName, OleDbConnection c)
        {
            ...
            try
            {
                ...
                using (var dataCommand = new OleDbCommand(sQuery, c))
                {
                    dataCommand.CommandTimeout = 60;
                    var dataAdapter = new OleDbDataAdapter(dataCommand);
            ...
            (string queryText, string dbConnectionString, CancellationToken token)
        {
            ...
            using (var c = new OleDbConnection(dbConnectionString))
            {
                try
                {
                    ...
                    dataCommand = new OleDbCommand(queryText, c);
        ...
        private void c_InfoMessage(object sender, OleDbInfoMessageEventArgs e)
        {
            string errmsg = string.Empty;
            foreach (OleDbError err in e.Errors)
            {
                errmsg += "[" + err.Source + "]" + err.Message + "\n";
            }
